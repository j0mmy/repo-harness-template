# Portable Agent Harness Template — Design Specification

> **Note:** This is the original, full design specification and rationale for the harness.
> The repository root contains the implemented scaffold. Where this document shows
> stack-specific examples (pytest, ruff, mypy, `.venv`), the shipped scaffold uses
> Python-stdlib equivalents (`unittest`, syntax lint) so the template verifies with
> GNU make and `python3` alone; swap your stack's tools back in via the stable
> Makefile target names. Usage docs live in the root `README.md`.
